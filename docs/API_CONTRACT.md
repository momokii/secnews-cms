...
fill/enrich/source-draft prompt text comes from the ADMIN-managed template per
kind (#55/#56, default = the best-practice built-in security-intelligence
prompts); the engine substitutes the placeholder legend (below) with the
...
| 33b | `POST /tickets/:id/ai/source-draft` | WORK | `SourceDraftBodySchema` `{sourceIds: uuid[] (1-20), targetFields: subset of overview\|description\|recommendations\|references (≥1), allowWebSearch? (default false), provider?, model?}` — the analyst picks the grounding sources and which narrative fields to draft | 200 `AiFillResponseSchema` — suggestions grounded ONLY on the chosen sources, stored as PENDING rows with provider + model; appends an `AI_SOURCE_DRAFT` activity entry `<n> suggestion(s) from <m> source(s)` | 400 `VALIDATION` empty `sourceIds`/`targetFields`, >20 sources, or a target field outside the four narrative fields (typed working fields like `cveIds` are not draftable here); 404 `NOT_FOUND` unknown ticket or a `sourceId` not belonging to the ticket; 422/502 as #32. `allowWebSearch: true` adds a references-only web-search instruction to the prompt (prompt augmentation only — no server-side fetching; the model may use browsing if its runtime provides it) |
...
| 55 | `GET /prompts` | ANY | — | 200 `[{kind: "FILL"\|"ENRICH"\|"SOURCE_DRAFT", content, updatedAt (null = built-in default, never edited), placeholders: [{name, description}]}]` — one item per kind. Serves the stored ADMIN template per kind, or the best-practice built-in default when the row is missing. `placeholders` is the legend of variables the engine injects: `{{ticketContext}}` (title, summary, findingType, tlp, iocs, sources — lines only when non-empty), `{{missingFields}}` (comma-joined strict fill scope), `{{currentFields}}` (`<field>: <value or <empty>>` per suggestible field), `{{selectedSources}}` (source-draft only: numbered title/url/notes list of the analyst-chosen grounding sources), `{{targetFields}}` (source-draft only: comma-joined requested output fields) | |
| 56 | `PUT /prompts/:kind` | ADMIN | `{content}` (non-empty, kind `FILL\|ENRICH\|SOURCE_DRAFT`; 400 otherwise) | 200 `PromptTemplateSchema` (upserted row); every successful PUT atomically appends a revision `{promptKind,content,actorId,createdAt}`. #32/#33/#33b render this stored template by substituting the placeholder legend above with the ticket's actual data; unknown `{{...}}` text passes through untouched. Missing row → best-practice built-in default. In the SOURCE_DRAFT flow `{{ticketContext}}` binds WITHOUT the all-sources line (chosen sources are injected only via `{{selectedSources}}` — unselected evidence must not leak into a grounded draft) | 403 non-ADMIN; 400 empty content / unknown kind |
